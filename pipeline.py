...
from trainer import DataArguments, ModelArguments, CrossNDTrainer
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(model_path,  trust_remote_code=True)
    model = Qwen3ForCrossND.from_pretrained(model_path, torch_dtype=dtype ,config=config, trust_remote_code=True,attn_implementation="flash_attention_2").cuda()
    ...
    training_args.include_for_metrics = ['metadata']
    training_args.remove_unused_columns = False
    # 创建训练器
    trainer = CrossNDTrainer(
        model=model,
        args=training_args,
