...
RUN_NAME="qwen4b-cls-single-source"

# 设置训练设备
DEEPSPEED_GPUS="localhost:0,1"
...
OUTPUT_DIR="output/qwen4b-cls-single-source"
NUM_TURN=1
...
# 运行训练命令
deepspeed --master_port 12345 --include $DEEPSPEED_GPUS \
    pipeline.py \
    --run_name $RUN_NAME \
    --use_outer false \
    --data_dir $DATA_DIR \
    ...
    --bf16 > output-single-source.log 2>&1