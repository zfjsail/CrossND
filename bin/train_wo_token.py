...
def main():
    ...
    if model_args.use_binary_head:
        ...
        if model_args.use_hybrid_head:
            model.add_hybrid_head()
            model_args.modules_to_save += ["hybrid_lm_head"]
        model_args.label_type = 'hard'
    ...
    # model.LABEL_TOKEN_IDS = torch.tensor(tokenizer.convert_tokens_to_ids(LABEL_TOKEN))
    model.set_token(tokenizer)
